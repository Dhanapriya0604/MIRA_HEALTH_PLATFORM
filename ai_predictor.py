...
def get_risk_level(glucose, haemoglobin, cholesterol):
    risk_score = 0
    if glucose > 126: risk_score += 3
    elif glucose > 99: risk_score += 1
    if haemoglobin < 8: risk_score += 3
    elif haemoglobin < 12: risk_score += 2
    if cholesterol > 240: risk_score += 3
    elif cholesterol > 200: risk_score += 1
    if risk_score >= 5: return "🔴 High Risk"
    elif risk_score >= 2: return "🟡 Moderate Risk"
    else: return "🟢 Healthy"